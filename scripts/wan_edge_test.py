...
def run_edge_tests():
    ...
    print("   Goal: Find hardware limits for resolution, frames, aspect ratios")
...
def print_summary(results):
    ...
    if successful:
        ...
        for r in sorted(
            successful, key=lambda x: x["pixels"] * x["frames"], reverse=True
        ):
            print(
                f"   {r['width']}x{r['height']} @ {r['frames']}f = {r['time_sec']}s ({r['description']})"
            )
        ...
        print("\n📐 DISCOVERED LIMITS:")
        print(
            f"   Max Resolution: {max_res['width']}x{max_res['height']} ({max_res['pixels']} pixels)"
        )
        print(
            f"   Max Frames: {max_frames['frames']} frames @ {max_frames['width']}x{max_frames['height']}"
        )
    ...
    if failed:
        ...
        for r in failed:
            print(
                f"   {r['width']}x{r['height']} @ {r['frames']}f ({r['description']})"
            )

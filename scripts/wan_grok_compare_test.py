    ...
    for nid, node in workflow.items():
        ...
        if node.get("class_type") == "VHS_VideoCombine":
            node["inputs"]["filename_prefix"] = (
                f"grok_compare_{width}x{height}_{frames}f"
            )
...
def queue_prompt(workflow):
    ...
    req = urllib.request.Request(
        f"{COMFY_API}/prompt", data=data, headers={"Content-Type": "application/json"}
    )
...
if __name__ == "__main__":
    print(
        f"🎬 Grok Compare Test: {WIDTH}×{HEIGHT} @ {FRAMES} frames ({FRAMES / 16:.1f}s @ 16fps)"
    )
    print("📊 Grok specs: 464×688 @ 144 frames (6s @ 24fps) = 46M pixels")
    print(f"📊 Our test: {WIDTH * HEIGHT * FRAMES / 1_000_000:.1f}M pixels")
    print("📊 GPU allocation: cuda:0,12gb;cuda:1,16gb;cpu,*")
    ...
    print("⏳ Monitor: watch -n5 'nvidia-smi --query-gpu=memory.used --format=csv'")

...
def run_gpu_only_test():
    ...
    print("   Mode: NO CPU offloading - pure GPU")
    ...
    for width, height, frames, description in TESTS:
        ...
        if status == "success":
            ...
            print(f"     Peak VRAM: {peak0} + {peak1} = {peak0 + peak1}MB")
            print("     💡 T5 encoder needs ~8GB, must use CPU offload for this config")
    ...
    if successful:
        ...
        for r in successful:
            print(
                f"     {r['width']}x{r['height']} @ {r['frames']}f = {r['time_sec']}s"
            )

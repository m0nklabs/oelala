...
class WanTestSuite:
    ...
    def run_single_test(
        self, width: int, height: int, frames: int, aspect_ratio: str
    ) -> TestResult:
        ...
        for r in self.results:
            if r.width == width and r.height == height and r.frames == frames:
                print("  ⏭️ Already tested, skipping")
                return r
        ...
        for aspect, resolutions in ASPECT_RATIOS.items():
            ...
            for width, height in test_resolutions:
                ...
                if result.status == "success":
                    ...
                    print(
                        f"  🛑 OOM at {width}x{height} ({pixels} pixels) - skipping larger resolutions"
                    )
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "failed": len(failed),
            "max_resolution": f"{max_resolution.width}x{max_resolution.height}"
            if max_resolution
            else None,
            "max_frames": max_frames.frames if max_frames else None,
            "avg_generation_time": sum(r.generation_time_sec for r in successful)
            / len(successful)
            if successful
            else 0,
            "working_resolutions": [
                {
                    ...
                    "time": r.generation_time_sec,
                }
                for r in sorted(
                    successful, key=lambda r: r.width * r.height, reverse=True
                )
            ],
        }
...
"""
        for r in self.results:
            status_icon = (
                "✅" if r.status == "success" else "❌" if r.status == "oom" else "⚠️"
            )

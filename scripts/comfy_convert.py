...
def convert_workflow_to_api(workflow_path: str, output_path: str = None):
    ...
    links = {
        l[0]: {"from": str(l[1]), "from_slot": l[2], "to": str(l[3]), "to_slot": l[4]}
            ...
            for inp in node["inputs"]:
                if inp.get("link"):
                    ...
                    if link:
                        ...
                        log(
                            f"  Linked: {inp['name']} <- node {link['from']} slot {link['from_slot']}"
                        )
                ...
                for wname in widget_names:
                    if widget_idx < len(widgets):
                        ...
                        if val is not None and not (
                            isinstance(val, dict) and "type" in val
                        ):
                            inputs[wname] = val
    ...
    for node_id, node in api.items():
        ...
        for input_name in required:
            if input_name not in node["inputs"]:
                ...
                if (
                    len(required[input_name]) < 2
                    or "default" not in required[input_name][1]
                ):
                    issues.append(
                        f"Node {node_id} ({node_type}): Missing required input '{input_name}'"
                    )

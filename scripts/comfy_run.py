...
class ResourceMonitor:
    ...
    def _monitor_loop(self):
        while self.running:
            ...
            self.data.append({"time": timestamp, "gpus": gpus, **cpu_ram})
            ...
            if gpus:
                ...
                print(
                    f"\r  GPU0: {gpu0:,}MB | GPU1: {gpu1:,}MB | RAM: {cpu_ram['ram_used_mb']:,}MB",
                    end="",
                    flush=True,
                )
    ...
    def get_summary(self):
        ...
        gpu0_peak = max(
            (d["gpus"][0]["mem_used_mb"] for d in self.data if d["gpus"]), default=0
        )
        gpu1_peak = max(
            (d["gpus"][1]["mem_used_mb"] for d in self.data if len(d["gpus"]) > 1),
            default=0,
        )
        ram_peak = max(d["ram_used_mb"] for d in self.data)

        gpu0_total = (
            self.data[0]["gpus"][0]["mem_total_mb"] if self.data[0]["gpus"] else 16384
        )
        gpu1_total = (
            self.data[0]["gpus"][1]["mem_total_mb"]
            if len(self.data[0]["gpus"]) > 1
            else 12288
        )
...
"""
    ...
    def export_csv(self, filename):
        with open(filename, "w") as f:
            f.write(
                "timestamp,gpu0_mem_mb,gpu0_util,gpu1_mem_mb,gpu1_util,ram_mb,cpu_load\n"
            )
            for d in self.data:
                ...
                gpu1 = (
                    d["gpus"][1]
                    if len(d["gpus"]) > 1
                    else {"mem_used_mb": 0, "util_pct": 0}
                )
                f.write(
                    f"{d['time']},{gpu0['mem_used_mb']},{gpu0['util_pct']},{gpu1['mem_used_mb']},{gpu1['util_pct']},{d['ram_used_mb']},{d['cpu_load']}\n"
                )
        return filename
...
def run_workflow(api_workflow_path):
    ...
    # Export data
    csv_file = (
        f"/home/flip/oelala/comfy_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )

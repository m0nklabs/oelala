...
WORKFLOW_PATH = Path(
    "/home/flip/oelala/ComfyUI/user/default/workflows/WAN2.2-I2V-DisTorch2-NEW_api.json"
)
...
class HardwareLimits:
    ...
    def update_from_result(self, result: BenchmarkResult):
        ...
        if result.status == "success":
            ...
            if (
                result.pixels
                > self.limits_by_aspect[ar]["max_res"][0]
                * self.limits_by_aspect[ar]["max_res"][1]
            ):
            ...
            if (
                self.oom_pixel_threshold == 0
                or result.pixels < self.oom_pixel_threshold
            ):
                self.oom_pixel_threshold = result.pixels
            if (
                self.oom_frame_threshold == 0
                or result.frames < self.oom_frame_threshold
            ):
...
class WanBenchmark:
    ...
    def _generate_frontend_config(self):
        ...
        for ar, results in by_aspect.items():
            resolutions = sorted(
                set((r.width, r.height) for r in results), key=lambda x: x[0] * x[1]
            )
        ...
        if len(successful) >= 3:
            ...
            factors = [
                r.generation_time_sec / (r.pixels * r.frames)
                for r in successful
                if r.pixels * r.frames > 0
            ]
    ...
    def _should_skip(self, width: int, height: int, frames: int) -> Tuple[bool, str]:
        ...
        if self.limits.oom_pixel_threshold > 0:
            if pixels >= self.limits.oom_pixel_threshold and frames >= 101:
                return (
                    True,
                    f"Beyond OOM threshold ({self.limits.oom_pixel_threshold} pixels)",
                )
        ...
        if self.limits.oom_frame_threshold > 0:
            if frames >= self.limits.oom_frame_threshold and pixels >= 720 * 400:
                return (
                    True,
                    f"Beyond OOM threshold ({self.limits.oom_frame_threshold} frames)",
                )
    ...
    def _wait_for_completion(
        self, prompt_id: str, timeout: int = 1800
    ...
    def run_single_test(self, width: int, height: int, frames: int) -> BenchmarkResult:
        ...
        if status == "success":
            print(
                f"  ✅ SUCCESS {elapsed:.1f}s - {result.pixels_per_sec / 1e6:.2f}Mpix/s - VRAM: {peak0}+{peak1}MB"
            )
        elif status == "oom":
    ...
    def run_full_benchmark(self, quick: bool = False):
        ...
        print(f"\n{'=' * 60}")
        print("🚀 WAN 2.2 HARDWARE BENCHMARK v2")
    ...
    def print_summary(self):
        ...
        if successful:
            ...
            print(
                f"   ⚡ Fastest: {fastest.width}x{fastest.height}@{fastest.frames}f = {fastest.generation_time_sec:.1f}s"
            )

            # Highest quality (most pixels*frames)
            best = max(successful, key=lambda r: r.pixels * r.frames)
            print(
                f"   👑 Best Quality: {best.width}x{best.height}@{best.frames}f = {best.generation_time_sec:.1f}s"
            )

            # Best efficiency (pixels*frames per second)
            efficient = max(successful, key=lambda r: r.pixels_per_sec)
            print(
                f"   📈 Most Efficient: {efficient.width}x{efficient.height} = {efficient.pixels_per_sec / 1e6:.2f}Mpix/s"
            )
    ...
    def generate_charts(self):
        ...
        fig.suptitle(
            f"WAN 2.2 Hardware Benchmark - {GPU_SPECS['cuda:0']['name']} + {GPU_SPECS['cuda:1']['name']}",
            fontsize=14,
            fontweight="bold",
        )
        ...
        colors = [
            "green"
            if r.time_category in ["instant", "fast"]
            else "orange"
            if r.time_category == "normal"
            else "red"
            for r in successful
        ]
        ...
        ax3.axhline(
            ...
            color="red",
            linestyle="--",
            label=f"Total VRAM ({TOTAL_GPU_VRAM_MB}MB)",
        )

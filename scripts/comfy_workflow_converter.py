        ...
        for input_name, input_def in all_inputs.items():
            ...
            if isinstance(input_def, list) and len(input_def) > 0:
                ...
                if not (
                    ...
                    and input_type.isupper()
                    and len(input_type) > 2
                ):
            ...
            if isinstance(input_def, list) and len(input_def) > 0:
                ...
                if is_connection:
                    ...
                    for node_input in node.get("inputs", []):
                        if node_input.get("name") == input_name and node_input.get(
                            "link"
                        ):
                            link_info = link_map.get(node_input["link"])
                            if link_info:
                                inputs[input_name] = [
                                    str(link_info["from_node"]),
                                    link_info["from_slot"],
                                ]
                            break
                else:
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python comfy_workflow_converter.py <workflow.json> [output_api.json]"
        )
        print("\nExample:")
    ...
    if result:
        ...
        print("\nTo run via terminal:")
        print(
            f"  curl -X POST http://127.0.0.1:8188/prompt -H 'Content-Type: application/json' -d @'{result}'"
        )

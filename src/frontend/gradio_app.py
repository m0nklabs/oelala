...
def generate_text_to_video(prompt, num_frames, model_type):
    ...
    try:
        # Prepare form data
        data = {"prompt": prompt, "num_frames": num_frames, "model_type": model_type}
        ...
        if response.status_code == 200:
            ...
            if video_path and os.path.exists(video_path):
                return (
                    video_path,
                    f"✅ Video generated: {result.get('message', 'Success')}",
                )
            else:
...
def generate_image_to_video(image, prompt, num_frames):
    ...
    try:
        ...
        with open(temp_image_path, "rb") as f:
            ...
            data = {"prompt": prompt, "num_frames": str(num_frames)}
...
with gr.Blocks(title="WAN2.2 Video Generator") as demo:
    ...
    status_text = gr.Textbox(
        label="Backend Status", value=check_backend_status(), interactive=False
    )
    ...
    with gr.Tab("📝 Text-to-Video"):
        ...
        with gr.Row():
            t2v_frames = gr.Slider(
                label="Number of Frames", minimum=8, maximum=32, value=16, step=1
            )
            t2v_model = gr.Dropdown(
                label="Model Type", choices=["light", "wan2.2", "svd"], value="light"
            )

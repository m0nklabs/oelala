...
async def stripe_webhook(request: Request):
    ...
    try:
        if STRIPE_WEBHOOK_SECRET:
            ...
            logger.warning(
                "⚠️ Stripe webhook secret not configured - signature not verified"
            )
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if not user_id or not credits:
            logger.error(
                f"Missing data in webhook: user_id={user_id}, credits={credits}"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Added {credits} credits to user {user_id} (payment: {payment_intent})"
            )
        except Exception as e:

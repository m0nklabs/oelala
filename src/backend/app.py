...
Generate as JSON."""
    ...
    try:
        async with httpx.AsyncClient(
            timeout=30.0, headers=_guardian_headers()
        ) as client:
...
async def upscale_video(
    ...
    """
    logger.info(
        f"🎬 Video upscale request: model={model}, scale={scale}x, quality={quality_preset}"
    )
    ...
    try:
        ...
        if model == "seedvr2":
            ...
            esrgan_model = (
                "RealESRGAN_x4plus.pth" if scale == 4 else "RealESRGAN_x2plus.pth"
            )
            workflow = {
                "1": {
                    "inputs": {

...
class CreditManager:
    ...
    async def deduct(
        ...
        response = await client.post(
            ...
            json={
                "p_user_id": user_id,
                "p_amount": amount,
                "p_description": description or "Generation job",
        ...
        if result and len(result) > 0:
            ...
            debug_log(
                f"Deducted {amount} credits from user {user_id}, new balance: {result_row.get('new_balance')}, ref={reference_id}"
            )
        else:
    ...
    async def add_credits(
        ...
        new_lifetime = balance.lifetime_purchased + (
            amount if type == "purchase" else 0
        )
    ...
    async def get_packages(self) -> List[CreditPackageResponse]:
        ...
        return [CreditPackageResponse(**row) for row in response.json()]
